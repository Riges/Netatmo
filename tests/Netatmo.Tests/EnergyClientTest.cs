...
public class EnergyClientTest : IDisposable
{
    ...
    [Theory]
    [AutoDomainData]
    public async Task CreateHomeSchedule_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, EnergyClient sut)
    {
        var parameters = new CreateHomeScheduleRequest("5a327cbdb05a2133678b5d3e", 14, 16, "Cat schedule");
        var expectedResponse = new CreateHomeScheduleResponse("ok", 0.036107063293457, Instant.FromUnixTimeSeconds(1518023467), "5a819e6113475d09c28b497a");
        httpTest.RespondWithJson(expectedResponse);

        var result = await sut.CreateHomeSchedule(parameters);

        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/createnewhomeschedule")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
            .WithRequestJson(parameters)
            .Times(1);

        result.Should().NotBeNull();
        result.ScheduleId.Should().Be(expectedResponse.ScheduleId);
        result.Status.Should().Be(expectedResponse.Status);
        result.TimeExec.Should().Be(expectedResponse.TimeExec);
        result.TimeServer.ToString().Should().Be(expectedResponse.TimeServer.ToString());
    }

    [Theory]
    [AutoDomainData]
    public async Task DeleteHomeSchedule_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, string homeId, string scheduleId, EnergyClient sut)
    {
        var expectedResponse = new DataResponse("ok", 0.036107063293457, Instant.FromUnixTimeSeconds(1518023467));
        httpTest.RespondWithJson(expectedResponse);

        var result = await sut.DeleteHomeSchedule(homeId, scheduleId);

        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/deletehomeschedule")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task GetHomesData_Should_Return_DataResponse_With_HomesData([Frozen] ICredentialManager credentialManager, EnergyClient sut)
    {
        ...
        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/homesdata")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task GetHomeStatus_Should_Return_DataResponse_With_HomeStatus([Frozen] ICredentialManager credentialManager, string homeId, EnergyClient sut)
    {
        httpTest.RespondWith(
        ...
        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/homestatus")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task GetRoomMeasure_Should_Return_TemperatureSteps([Frozen] ICredentialManager credentialManager, EnergyClient sut)
    {
    ...
    [Theory]
    [AutoDomainData]
    public void GetRoomMeasure_With_Bad_Type_Should_Throw_ArgumentException(EnergyClient sut)
    {
    ...
    public async Task RenameHomeSchedule_Should_Return_Expected_Result(
        [Frozen] ICredentialManager credentialManager,
        string homeId,
        string scheduleId,
        string name,
        EnergyClient sut)
    {
        ...
        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/renamehomeschedule")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task SetRoomThermPoint_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, string homeId, string roomId, string mode, EnergyClient sut)
    {
        httpTest.RespondWithJson(new DataResponse("ok", 0.036107063293457, Instant.FromUnixTimeSeconds(1518023467)));

        await sut.SetRoomThermPoint(homeId, roomId, mode);

        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/setroomthermpoint")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task SetThermMode_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, string homeId, string mode, EnergyClient sut)
    {
        httpTest.RespondWithJson(new DataResponse("ok", 0.036107063293457, Instant.FromUnixTimeSeconds(1518023467)));

        await sut.SetThermMode(homeId, mode);

        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/setthermmode")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task SwitchHomeSchedule_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, string homeId, string scheduleId, EnergyClient sut)
    {
        httpTest.RespondWithJson(new DataResponse("ok", 0.036107063293457, Instant.FromUnixTimeSeconds(1518023467)));

        await sut.SwitchHomeSchedule(homeId, scheduleId);

        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/switchhomeschedule")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [Theory]
    [AutoDomainData]
    public async Task SyncHomeSchedule_Should_Return_Expected_Result([Frozen] ICredentialManager credentialManager, EnergyClient sut)
    {
        ...
        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/synchomeschedule")
            .WithVerb(HttpMethod.Post)
            .WithOAuthBearerToken(credentialManager.AccessToken)
            .WithContentType("application/json")
    ...
    [AutoDomainData]
    public void GetRoomMeasure_Should_Throw_ArgumentException(GetRoomMeasureParameters parameters, string exceptionMessage, EnergyClient sut)
    {
