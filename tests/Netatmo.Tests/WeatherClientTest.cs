...
public class WeatherClientTest : IDisposable
{
    ...
    [Theory]
    [AutoDomainData]
    public async Task GetStationsData_Should_Return_DataResponse_With_StationsData([Frozen] ICredentialManager credentialManager, string accessToken, WeatherClient sut)
    {
        credentialManager.AccessToken.Returns(accessToken);
        ...
        httpTest.ShouldHaveCalled("https://api.netatmo.local/api/getstationsdata")
